                ...
                <div class="filter-container">
                    <form action="" method="GET" class="filter-form" id="filterForm">
                        <div class="search-box">
                            <input type="text" id="searchInput" placeholder="Cari produk..." value="<?php echo isset($_GET['search']) ? htmlspecialchars($_GET['search']) : ''; ?>">
                        </div>
                        <select name="kategori" onchange="this.form.submit()">
                            <option value="">Semua Kategori</option>
                            <?php foreach ($kategori_list as $kat): ?>
                                <option value="<?php echo htmlspecialchars($kat); ?>" <?php echo (isset($_GET['kategori']) && $_GET['kategori'] === $kat) ? 'selected' : ''; ?>>
                        ...
                        <select name="sub_kategori" onchange="this.form.submit()">
                            ...
                            <?php foreach ($sub_kategori_list as $sub_kat): ?>
                                <option value="<?php echo htmlspecialchars($sub_kat); ?>" <?php echo (isset($_GET['sub_kategori']) && $_GET['sub_kategori'] === $sub_kat) ? 'selected' : ''; ?>>
            ...
            <div class="content">
                            ...
                            <?php if (!empty($products)): ?>
                                <?php foreach ($products as $product): ?>
                                    <tr>
                                        <td>
                                            <?php if (!empty($product['gambar_satu'])): ?>
                                                <img src="data:image/jpeg;base64,<?php echo base64_encode($product['gambar_satu']); ?>"
                                                    alt="Gambar Produk" style="width: 50px; height: 50px; object-fit: cover; border-radius: 4px;">
                                            <?php else: ?>
                                            ...
                                            <?php endif; ?>
                                        </td>                                        <td class="small-text">#<?php echo htmlspecialchars($product['product_id']); ?></td>
                                        <td class="small-text"><?php echo htmlspecialchars($product['nama_produk']); ?></td>
                                        <td class="small-text"><?php echo htmlspecialchars(substr($product['deskripsi'], 0, 50)) . '...'; ?></td>
                                        <td class="small-text">Rp <?php echo number_format($product['harga_produk'], 0, ',', '.'); ?></td>
                                        <td class="small-text">
                                            <span class="stock-badge <?php echo ($product['stok'] ?? 0) <= 5 ? 'low-stock' : 'normal-stock'; ?>">
                                        ...
                                        <td>
                                            <div class="aksi">
                                                <button onclick="window.location.href='edit_barang.php?id=<?php echo urlencode($product['product_id']); ?>'"
                                                    class="edit-btn small-btn">
                                                    <i class="fas fa-edit"></i>
                                                </button>
                                                <button onclick="if(confirm('Apakah Anda yakin ingin menghapus produk ini?')) window.location.href='hapus_barang.php?id=<?php echo urlencode($product['product_id']); ?>'"
                ...
                <div class="pagination">
                    <ul>
                        <?php if ($page > 1): ?>
                            <li>
                                <a href="?page=<?php echo $page - 1; ?><?php echo isset($_GET['kategori']) ? '&kategori=' . urlencode($_GET['kategori']) : ''; ?><?php echo isset($_GET['sub_kategori']) ? '&sub_kategori=' . urlencode($_GET['sub_kategori']) : ''; ?><?php echo isset($_GET['search']) ? '&search=' . urlencode($_GET['search']) : ''; ?>">
                        ...
                        <?php for ($i = 1; $i <= $total_pages; $i++): ?>
                            <li class="<?php echo ($i == $page) ? 'active' : ''; ?>">
                                <a href="?page=<?php echo $i; ?><?php echo isset($_GET['kategori']) ? '&kategori=' . urlencode($_GET['kategori']) : ''; ?><?php echo isset($_GET['sub_kategori']) ? '&sub_kategori=' . urlencode($_GET['sub_kategori']) : ''; ?><?php echo isset($_GET['search']) ? '&search=' . urlencode($_GET['search']) : ''; ?>">
                                    <?php echo $i; ?>
                                </a>
                            </li>
                        <?php endfor; ?>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function filterTable() {
                ...
                for (let i = 1; i < rows.length; i++) {
                    ...
                    const kategoriMatch = !selectedKategori || kategoriCell.textContent.toLowerCase() === selectedKategori;
                    const subKategoriMatch = !selectedSubKategori || subKategoriCell.textContent.toLowerCase() === selectedSubKategori;
